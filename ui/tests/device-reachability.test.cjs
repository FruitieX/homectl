...
vm.runInNewContext(ts.transpileModule(fs.readFileSync(path.join(__dirname,'../lib/deviceReachability.ts'),'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText,context);
...
test('reachable evidence is independent of scene confirmation and expires',()=>{
  const d=device({requested_at_ms:now,last_report:{received_at_ms:now-5000,retained:false,matches_requested:false}});
  assert.equal(health(d,now),'online');
  assert.equal(health(d,now+600_000),'stale');
});
test('cached reports do not establish reachability',()=>{
  assert.equal(health(device({last_report:{received_at_ms:now,retained:true}}),now),'unknown');
});
test('explicit offline wins unless a newer device report arrives',()=>{
  const d=device({availability:{online:false,observed_at_ms:now},last_report:{received_at_ms:now-10,retained:false}});
